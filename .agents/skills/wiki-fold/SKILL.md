---
name: wiki-fold
description: "Rollup of wiki log entries into meta-pages. Reads the specified recent entries from wiki/log.md, creates an extractive summary, and writes a fold page to wiki/folds/. Operates in a 2-step process: previews in chat first, then commits. Triggers on: fold the log, run a fold, run wiki-fold, log rollup, roll up log entries."
---

# wiki-fold: Extractive Log Rollup (MVP)

A "fold" is a periodic rollup of `wiki/log.md` entries to keep the log manageable and synthesize past work.
This skill operates strictly in a **2-Step Process** (Draft/Dry-run -> Commit) and requires **Extractive Summarization**.

**Trigger**: The user asks to fold, rollup, or compress the log, typically specifying a number of logs or a date range (e.g., "fold the last 10 logs" or "roll up logs from last week").

---

## 1. Step One: Draft / Dry-run

When the user triggers `wiki-fold`:
...
5. **Stop and Wait**: Do not create any files until the user explicitly approves.
